...
PATH_KDEGLOBAL="$PATHCONF/kdeglobals"
...
apply_polybar() {
	...
	# Update theme path references in modules.ini dynamically
	# First, create a temporary file to process theme path replacements
	MODULES_FILE="${PATH_PBAR}/modules.ini"
	TEMP_FILE=$(mktemp)
	
	# Process the file line by line
	cat "$MODULES_FILE" | while IFS= read -r line; do
		if [[ "$line" == *"~/.config/bspwm/themes/"* ]]; then
			# Extract current theme from the path
			current_path=$(echo "$line" | grep -o "~/.config/bspwm/themes/[^/]*")
			if [[ -n "$current_path" ]]; then
				current_theme=$(echo "$current_path" | sed 's|~/.config/bspwm/themes/||')
				# If the theme in the path is not the current theme, replace it
				if [[ "$current_theme" != "$THEME" ]]; then
					echo "$line" | sed "s|~/.config/bspwm/themes/$current_theme|~/.config/bspwm/themes/$THEME|g" >> "$TEMP_FILE"
				else
					echo "$line" >> "$TEMP_FILE"
				fi
			else
...
apply_rofi() {
	...
	# modify rofi scripts - find all scripts starting with rofi_ and update them
	find "${PATH_BSPWM}/scripts" -type f -name "rofi_*" -exec \
...
# KDE Globals -------------------------------
apply_kde_globals() {
    if [[ -f "${TDIR}/apply_kdeglobals.sh" ]]; then
        chmod +x "${TDIR}/apply_kdeglobals.sh"
        bash "${TDIR}/apply_kdeglobals.sh"
        
        # Copy kdeglobals to the appropriate location if it exists
        if [[ -f "${TDIR}/kdeglobals" ]]; then
            cp "${TDIR}/kdeglobals" "${HOME}/.config/kdeglobals"
            echo "Applied KDE global colors configuration"
        fi
    else
        echo "KDE globals script not found"
    fi
}
